...
(
  ...
  // Steering Logic
    ...
    wire hold_both_Dhl = inst_val_Dhl && (steering_mux_sel_Dhl == 1'b0) && !current_stall_Dhl && !stall_X0hl && !redirect_Dhl;

    always @(posedge clk) begin
        if (reset) begin
            steering_mux_sel_Dhl <= 1'b0; // reset to 0, meaning we will try to move the bottom instruction firstf
        end
  ...
  always @(*) begin
    ...
    instB_Dhl = 32'd0; //(steering_mux_sel_Dhl == 1'b1) ? ir0_Dhl : ir1_Dhl; 
  end
...
//   wire [4:0] rsB1_addr_Dhl = (steering_mux_sel_Dhl == 1'b1) ? inst0_rs2_Dhl : inst1_rs2_Dhl;
  ...
  wire redirect_Dhl = brj_taken_Dhl && issue_Dhl; // if we are taking a branch/jump and we are actually issuing an instruction, then we need to redirect
  ...
  reg [6:0] scoreboard [31:0]; // scoreboard (index by register, msb [6] bit is pending, fifth bit is functional unit,  0 to 4 bits are stage in pipeline)
  ...
  wire [6:0] sb_debug29 = scoreboard[5'd29][6:0];
...
// these are for A
  ...
  wire bubbleB_sel_Dhl = (squash_Dhl || stallB_Dhl || redirect_Dhl); // A may be a d-stage jump then we have to kill
...
//   wire stall_muldiv_X0hl = 1'b0; //( muldivreq_val_X0hl && inst_val_X0hl && !muldivresp_val ); TOOO SEE IF NEEDED
  ...
  assign stall_X0hl = ( stall_X1hl || stall_imem_X0hl || stall_dmem_X0hl ); // TODO: rmemoved stall_muldiv_X0hl
